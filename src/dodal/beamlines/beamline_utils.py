...
from ophyd_async.core import PathProvider, StaticPathProvider
...
def set_directory_provider(provider: PathProvider):
    global DIRECTORY_PROVIDER

    DIRECTORY_PROVIDER = provider


def get_directory_provider() -> PathProvider:
    if DIRECTORY_PROVIDER is None:
        set_directory_provider(
            StaticPathProvider(tempfile.NamedTemporaryFile().name, "")
        )
